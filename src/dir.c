...
static int fs_iterate(struct file *file, struct dir_context* ctx) {
        ...
        //      printk("%s: we are inside readdir with ctx->pos set to %lld\n", MOD_NAME, ctx->pos);
                if(!dir_emit(ctx,".", FILENAME_MAXLEN, FS_ROOT_INODE_NUMBER, DT_UNKNOWN)){
                        // printk("%s: return in ctx->pos set to %lld\n", MOD_NAME, ctx->pos);
                        return 0;
                }
                else{
                        ctx->pos++;
        ...
        //      printk("%s: we are inside readdir with ctx->pos set to %lld\n", MOD_NAME, ctx->pos);
                ...
                if(!dir_emit(ctx,"..", FILENAME_MAXLEN, 1, DT_UNKNOWN)){
                        // printk("%s: return in ctx->pos set to %lld", MOD_NAME, ctx->pos);
                        return 0;
                }
                else{
                        ctx->pos++;
        ...
        if (ctx->pos == 2){
        //      printk("%s: we are inside readdir with ctx->pos set to %lld\n", MOD_NAME, ctx->pos);
                if(!dir_emit(ctx, FILE_NAME, FILENAME_MAXLEN, FS_UNIQFILE_INODE_NUMBER, DT_UNKNOWN)){
                        // printk("%s: return in ctx->pos set to %lld\n", MOD_NAME, ctx->pos);
                        return 0;
                }
                else{
